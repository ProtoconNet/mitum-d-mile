...
import (
	...
	dmilestate "github.com/ProtoconNet/mitum-d-mile/state"
...
func (opp *RegisterModelProcessor) PreProcess(
	...
	if found, _ := state.CheckNotExistsState(dmilestate.DesignStateKey(fact.Contract()), getStateFunc); found {
		return ctx, mitumbase.NewBaseOperationProcessReasonError(
			common.ErrMPreProcess.
				Wrap(common.ErrMServiceE).Errorf("dmile service for contract account %v",
				fact.Contract(),
			)), nil
	}
	...
	fmt.Println("1111111111111111112")
	return ctx, nil, nil
}
...
func (opp *RegisterModelProcessor) Process(
	...
	sts = append(sts, state.NewStateMergeValue(
		dmilestate.DesignStateKey(fact.Contract()),
		dmilestate.NewDesignStateValue(design),
	))
