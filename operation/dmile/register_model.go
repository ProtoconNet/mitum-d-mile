...
import (
	...
	dmiletypes "github.com/ProtoconNet/mitum-d-mile/types"
...
func (fact RegisterModelFact) IsValid(b []byte) error {
	...
	if len(fact.project) > dmiletypes.MaxProjectIDLen {
		return common.ErrValOOR.Wrap(
			errors.Errorf("project length over allowed, %d > %d", len(fact.project), dmiletypes.MaxProjectIDLen))
	}
